...
def cnn_model_fn(features, labels, mode, num_classes=20):
    ...
    with tf.variable_scope('vgg_16'):
        ...
        ckpt_name_to_var = dict()
        for v in variables_to_restore:
	    tmp_name = ''
            if 'kernel' in v.name:
                tmp_name = v.name.split('kernel')[0]+'weights'
            elif 'bias' in v.name:
                tmp_name = v.name.split('bias')[0]+'biases'
	    else:
		continue
	    ckpt_name_to_var[tmp_name] = v
	print(ckpt_name_to_var)
        tf.train.init_from_checkpoint('./vgg_16.ckpt', ckpt_name_to_var)
...
def main():
    ...
    pascal_classifier = tf.estimator.Estimator(
        ...
        model_dir="/tmp/pascal_model_finetune_vgg16_LRe-3_noSubMean")
